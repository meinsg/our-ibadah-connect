...
const PrayerTimes = () => {
  ...
  const { location, loading: locationLoading, refetch: refetchLocation } = useSharedLocation();
    ...
    <Card className="p-4 sm:p-6 shadow-prayer bg-spiritual border-accent">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          <div className="text-xs sm:text-sm text-muted-foreground">{formatTime(currentTime)}</div>
          <Button
            variant="ghost"
            size="sm"
        ...
        {prayerTimes.map((prayer) => (
          <div
            ...
            className={`flex items-center justify-between p-2 sm:p-3 rounded-lg transition-all touch-manipulation ${
              prayer.isNext ? "bg-gradient-spiritual text-primary-foreground shadow-soft" : "bg-accent/50 text-foreground"
            }`}
          >
