...
const Mosques = () => {
  ...
  const { places, loading: placesLoading, error: placesError, searchMosques } = usePlaces();
  ...
  }, [location, radius, openNow, searchMosques]);
  ...
  const handleAutocompleteSelect = async (lat: number, lon: number, city?: string, country?: string) => {
    const address = city ? city + (country ? `, ${country}` : "") : `${lat},${lon}`;
      ...
      <header className="bg-background/80 backdrop-blur-sm border-b border-accent safe-area-top">
        <div className="container mx-auto px-4 py-3 sm:py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-primary font-amiri">Find Mosques</h1>
              <p className="text-xs text-muted-foreground font-inter hidden sm:block">
                Locate nearby mosques with prayer times and facilities
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="p-4 mb-6">
          <div className="space-y-4">
            ...
            <Button onClick={handleSearch} disabled={locationLoading || placesLoading} className="w-full sm:w-auto font-inter touch-manipulation">
              ...
              {locationLoading ? "Getting Location..." : placesLoading ? "Searching..." : "Search Mosques"}
            </Button>
          </div>
        </Card>
